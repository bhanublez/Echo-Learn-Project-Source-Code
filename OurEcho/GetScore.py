...
trainer_SST_lambda['en'] = Pro_T.getTrainer('en')
...
def lambda_handler(event):
    ...
    try:
        ...
        return {
            'statusCode': 400,
            'body': json.dumps({'error': 'Invalid Base64 string: ' + str(e)})
        }

    language = data.get('language', 'en')  # Default to 'en' if language is not provided
    if isinstance(real_text, list):
    ...
    #Return the sample response for now
